...
BlockController.prototype.getBlockByHash = function (hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.bitstashd.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.bitstashd.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype.blockIndex = function (req, res) {
    ...
    this.node.services.bitstashd.getBlockHeader(parseInt(height), function (err, info) {
        if (err) {
            return self.common.handleErrors(err, res);
        }
        res.jsonp({
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
    ...
    if (summaryCache) {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.bitstashd.client.getBlock(hash, function (err, response) {
            ...
            var confirmations = self.node.services.bitstashd.height - block.height + 1;
...
BlockController.prototype.list = function (req, res) {
    ...
    return self.node.services.bitstashd.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
